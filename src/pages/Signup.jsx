...
export const Signup = () => {
    ...
    const apicall=async(e)=>{
        ...
        if (mobile.visible && mobile.value) {
          ...
          } else if (name.visible && name.value) {
           
                setname((prev) => ({ ...prev, visible: false }));
       ...
       useEffect(() => {
        if (mobile.visible) {
          ...
          animateField(nameInputRef,sendbutton);
  ...
  return (
    <div className='w-full h-[100vh]  bg-gradient-to-t from-gray-900 via-gray-600 to-black flex justify-center items-center overflow-hidden fixed'>
        ...
        <div className='w-[30vh] h-[30vh] rounded-3xl bg-indigo-500 absolute rotate-180' ref={circle4} ></div> 
            ...
            <header className='uppercase text-2xl font-bold underline mt-6 text-blue'>Register</header>
       ...
       <div className='flex justify-center'><Link to="/login" className="uppercase text-center mt-4 block bg-indigo-600 p-2 text-white rounded-lg">login</Link></div>
            </form>
            </div>
